...
const STATUS_COLOR = { PENDING: '#f59e0b', PAID: '#22c55e', SHIPPED: '#4f46e5', DELIVERED: '#6b7280', CANCELLED: '#ef4444' }
...
function OrderPage() {
  ...
  if (loading) return <div style={{ textAlign: 'center', padding: '80px', color: '#6b7280' }}>불러오는 중...</div>
  ...
  return (
    <div style={{ maxWidth: '700px', margin: '0 auto' }}>
      ...
      {orders.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '60px', color: '#6b7280' }}>주문 내역이 없습니다.</div>
      ) : (
        orders.map((order) => (
          <div key={order.id} style={{ background: '#fff', borderRadius: '12px', marginBottom: '16px', boxShadow: '0 2px 10px rgba(0,0,0,0.06)', overflow: 'hidden' }}>
            <div
            ...
            >
              <div>
                ...
                <p style={{ fontSize: '13px', color: '#6b7280' }}>{order.orderedAt}</p>
              </div>
              <div style={{ textAlign: 'right' }}>
                ...
                <p style={{ fontWeight: '700', color: '#4f46e5' }}>{order.totalPrice.toLocaleString()}원</p>
              </div>
            </div>
            {expanded === order.id && (
              <div style={{ borderTop: '1px solid #f3f4f6', padding: '16px 20px', background: '#f9fafb' }}>
                <p style={{ fontSize: '13px', color: '#6b7280', marginBottom: '12px' }}>배송지: {order.address}</p>
